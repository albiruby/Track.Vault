...
export function ExportCardControls({
 ...
 : "bg-slate-50 border-transparent text-slate-755 hover:bg-slate-100"
 }`}
 >
 <span>{t.name}</span>
 ...
 <div className="space-y-3">
 <label className="text-xs font-mono font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2">
 <Palette className="w-4 h-4 text-blue-650" /> Color Aesthetics
 </label>
 ...
 theme === th.id
 ? "ring-2 ring-blue-500 ring-offset-2 border-transparent shadow-sm"
 ...
 {size === sz.id && <Check className="w-4 h-4 text-blue-400" />}
 </button>
 ))}
 </div>
 </div>
 ...
 disabled={exporting}
 className={`w-full py-3.5 px-4 rounded-xl font-bold text-sm tracking-wide shadow-md shadow-blue-500/10 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white transition-all transform active:scale-[0.98] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed`}
 >
 {exporting ? (
 <>
