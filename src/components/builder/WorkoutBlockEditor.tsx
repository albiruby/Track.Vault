...
export function WorkoutBlockEditor({ label, blocks, onChange, defaultType }: WorkoutBlockEditorProps) {
  ...
  return (
    <div className="bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] rounded-2xl p-6 space-y-4 shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
        <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-[#0F172A] dark:text-[#F8FAFC] flex items-center gap-2">
          <Footprints className="w-4 h-4 text-sky-500" /> {label} ({blocks.length})
        </h3>
        <button
          type="button"
          onClick={addBlock}
          className="px-3.5 py-1.5 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-bold text-xs tracking-wide flex items-center gap-1 transition-all shadow-sm active:scale-[0.98] cursor-pointer"
        >
      ...
      {blocks.length === 0 ? (
        <div className="py-6 text-center text-xs text-slate-400 italic border border-dashed border-[#E2E8F0] dark:border-[#334155] rounded-xl bg-slate-55/30 dark:bg-slate-900/10">
        ...
        <div className="space-y-4">
          {blocks.map((block, index) => (
            <div
              key={block.id}
              className="p-4 bg-slate-50/55 dark:bg-[#0F172A]/40 rounded-2xl border border-[#E2E8F0] dark:border-[#334155] space-y-3 relative group"
            >
              {/* Position and Removal Tooling */}
              <div className="flex justify-between items-center bg-slate-100 dark:bg-[#0F172A] p-2 rounded-xl border border-[#E2E8F0] dark:border-[#334155]">
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] font-mono font-bold text-[#0F172A] dark:text-slate-300 px-1.5">
                    STEP #{index + 1}
                  </span>
                  <select
                    value={block.type}
                    onChange={(e) => updateBlockField(block.id, { type: e.target.value })}
                    className="p-1 px-2 text-[10px] uppercase font-mono font-bold bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] rounded-lg text-[#0F172A] dark:text-slate-300 focus:outline-none cursor-pointer"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div className="space-y-1">
                  <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-355 block">Block Name</span>
                  <input
                    type="text"
                    value={block.name}
                    onChange={(e) => updateBlockField(block.id, { name: e.target.value })}
                    placeholder="e.g. Threshold interval"
                    className="w-full p-2 bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] text-xs rounded-xl text-[#0F172A] dark:text-[#F8FAFC]"
                  />
                </div>

                <div className="space-y-1">
                  <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-355 block">Repetitions</span>
                  <input
                    type="number"
                    min="1"
                ...
                <div className="space-y-1 col-span-1 sm:col-span-2">
                  <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-355 block">Work Target Type</span>
                  <div className="flex gap-2">
                    <select
                      value={block.work.targetType}
                      onChange={(e) => updateWorkField(block.id, { targetType: e.target.value as any })}
                      className="p-2 bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] text-xs rounded-xl text-slate-700 dark:text-slate-300 cursor-pointer font-sans"
                    >
                    ...
                    {block.work.targetType === "duration" ? (
                      <input
                        ...
                        className="flex-1 p-2 bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] text-xs rounded-xl text-[#0F172A] dark:text-[#F8FAFC]"
                      />
                ...
                <div className="space-y-1">
                  <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-355 block">Work Intensity</span>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-355 block">Rest Recovery Type</span>
                  <select
                    value={block.recovery?.type || "none"}
                    onChange={(e) => updateRecoveryField(block.id, { type: e.target.value })}
                    className="w-full p-2 bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] text-xs rounded-xl text-slate-750 dark:text-slate-200 cursor-pointer font-sans"
                  >
                ...
                {block.recovery && block.recovery.type !== "none" && (
                  <>
                    <div className="space-y-1 col-span-1">
                      <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-355 block">Rest Stop Target</span>
                      <div className="flex gap-1.5">
                        <input
                          ...
                          className="w-1/2 p-2 bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] text-xs rounded-xl text-[#0F172A] dark:text-[#F8FAFC]"
                        />
                        <input
                          type="number"
                          placeholder="Mtrs"
                    ...
                    <div className="space-y-1">
                      <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-355 block">Rest Intensity</span>
                      <input
                        type="text"
                        ...
                        className="w-full p-2 bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] text-xs rounded-xl text-[#0F172A] dark:text-slate-300"
                      />
                    </div>
                  </>
                )}
                ...
                <div className="space-y-1 col-span-1 sm:col-span-2 md:col-span-4">
                  <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-355 block">Block execution cues / notes (optional)</span>
                  <input
                    type="text"
                    value={block.notes || ""}
