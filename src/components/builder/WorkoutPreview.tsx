...
export function WorkoutPreview({ workout }: WorkoutPreviewProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 space-y-6">
      {/* Header Banner */}
      <div className="border-b border-slate-100 dark:border-slate-800 pb-5">
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <RiskBadge risk={risk} />
          <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded text-[10px] font-mono border border-slate-200 dark:border-slate-700 uppercase font-semibold">
            {workout.surface || "Road"}
          </span>
        </div>

        <h2 className="text-xl lg:text-2xl font-black font-display text-slate-800 dark:text-slate-100 tracking-tight">
          {title}
        </h2>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-4 leading-relaxed italic pr-4">
          "{summary}"
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-3 bg-slate-50 dark:bg-slate-800/20 rounded-xl border border-slate-200/40 dark:border-slate-800 text-center flex flex-col items-center">
          ...
          <span className="text-sm font-bold text-slate-800 dark:text-slate-200 font-mono mt-0.5">
            {duration} min
          </span>
        </div>
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-800/20 rounded-xl border border-slate-200/40 dark:border-slate-800 text-center flex flex-col items-center">
          ...
          <span className="text-[11px] font-bold text-slate-800 dark:text-slate-200 truncate mt-0.5 max-w-full">
      ...
      <div className="space-y-6">
        {/* Warmup Section */}
        <div>
          <h4 className="text-xs font-mono font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-1.5 mb-2.5">
            1. Warm-Up Blocks
          </h4>
          <div className="space-y-1.5 text-xs font-mono">
            {workout.warmup && workout.warmup.length > 0 ? (
              workout.warmup.map((block, i) => (
                <div key={block.id || i} className="p-2.5 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800 flex justify-between gap-2.5 items-center">
        ...
        {/* Main Set Section */}
        <div>
          <h4 className="text-xs font-mono font-bold uppercase tracking-wider text-orange-500 border-b border-orange-100 dark:border-orange-900/10 pb-1.5 mb-2.5">
            2. Main Set Blocks
          </h4>
          <div className="space-y-1.5 text-xs font-mono">
            {workout.mainSet && workout.mainSet.length > 0 ? (
              workout.mainSet.map((block, i) => (
                <div key={block.id || i} className="p-3 bg-orange-500/[0.03] dark:bg-orange-500/[0.01] border border-orange-500/10 dark:border-orange-500/5 rounded-xl flex flex-col gap-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800 pt-6">
        {workout.coachingNotes && workout.coachingNotes.length > 0 && (
          <div className="p-4 bg-emerald-50/[0.15] dark:bg-emerald-950/5 border border-emerald-500/10 rounded-2xl">
            <span className="text-[10px] font-mono uppercase tracking-widest text-emerald-600 dark:text-emerald-400 font-bold block mb-2">
        ...
        {workout.safetyNotes && workout.safetyNotes.length > 0 && (
          <div className="p-4 bg-rose-50/[0.15] dark:bg-rose-950/5 border border-rose-500/10 rounded-2xl">
            <span className="text-[10px] font-mono uppercase tracking-widest text-rose-600 dark:text-rose-400 font-bold block mb-2">
              🛡 Safety Warnings
            </span>
            <ul className="space-y-1.5 text-xs text-slate-600 dark:text-slate-300">
              {workout.safetyNotes.map((s, i) => (
                <li key={i} className="flex gap-1.5 leading-relaxed text-rose-750 dark:text-rose-300">
                  <span>⚠</span>
                  <span>{s}</span>
                </li>
              ))}
      ...
      {/* Disclaimer block */}
      <div className="text-center bg-slate-50 dark:bg-slate-850 p-3 rounded-xl border border-slate-100 dark:border-slate-800 text-[10px] text-slate-400">
